 ...
 * Added more filter options: filter_fields, filter_values, filter_operators
 * Date: 15/08/2014
 * Added more filter options filters={field,operations,value} where opration may be 'eq','neq','lw','gt','teq', 'tneq', 'taprox'. Example
 */
$DEBUG = false; 
...
class ArrestMySQL {
    ...
    private function read()
    {
        ...
        if($GLOBALS['DEBUG'])
        {
            echo 'read(): contenido de filter_fields: ' .$this->_get('filter_fields').'.<br/>';
            echo 'read(): contenido de filter_values: ' .$this->_get('filter_values').'.<br/>';
            echo 'read(): contenido de filter_operators: ' .$this->_get('filter_operators').'.<br/>';
            
            echo 'read(): contenido de filters: '.$this->_get('filters').'.<br/>';
         
        }
        // Pretratamiento del campo filters. Si viene informado, hayq ue hacer un explode para pasar la informacion partida
        if($this->_get('filters')!='')
        {
            $parameters = $this->_get('filters');
            if($GLOBALS['DEBUG'])
                echo 'read(): Pretratamiento de filtros<br/>';
            $parameters = str_replace('{{', '', $parameters);
            $parameters = str_replace('}}', '', $parameters);
            
            $filtros = explode('},{', $parameters );
        }
        ...
        } else if( $this->_get('filters')==''){
            $this->db->select('*')
                     ->from($table)
                     ->where($this->_get('filter_fields'), $this->_get('filter_values')) // Dani
        ...
        else if ($this->_get('filters')!=''){ // Dani: Multifilter mode
            $this->db->select('*')
                     ->from($table)
                     ->whereFiltered($filtros) // Dani
                     ->order_by($this->_get('order_by'), $this->_get('order'))
                     ->limit(intval($this->_get('limit')), intval($this->_get('offset')))
                     ->query();
            if($result = $this->db->fetch_all()){
