 ...
 * Added control for empty filter fields, as now the WHERE clause is always called
 * Date: 15/08/2014
 */
...
class Database {
    ...
    public function where($fields, $values, $operators = '') {
        ...
        if(!is_array($fields) && !is_array($values)) {
            if($GLOBALS['DEBUG'])
            {
                echo 'where(..). Fields and values are NOT array.<br/>';
            }
            // Added check to detect emty filter fields
            if($fields!="")
            {
            ...
            if($GLOBALS['DEBUG'])
            {
                echo 'where(..). Fields and values ARE array.<br/>';
            }
    ...
    /*
     * This functions reveice a list of filter triplets 'fieldname, fieldvalue, logic operation'
     */
    public function whereFiltered($filters) {

            if($GLOBALS['DEBUG'])
            {
                echo 'whereFiltered(..). '.$filters.'<br/>';
            }
            //$array = array_combine($fields, $values);
            $query = " WHERE ";
            ...
            foreach($filters as $filter) {
                ...
                $operator = explode(',', $filter)[1];
                 $value = explode(',', $filter)[2];
                
                if($GLOBALS['DEBUG'])
                {
                    echo '$key: '.$key.'. $value: '.$value.'.$operator: '.$operator.'.<br/>';
                }
                switch($operator)
                {
                    case "eq":
                    ...
                    case "taprox":
                        ...
                        $data[$counter] = 'upper(`' . $key . '`) LIKE \'%' . $value . '%\' ';    
                        break;
                    default:
                        echo 'ERROR: operation '.$operator.' unknown.<br/>';
